...
**이 repo 의 `data/gt/` 는 46개 전부 손질이 끝났다.** U+FFFD 는 남아 있지 않고,
텍스트 레이어가 없는 `ch/(2020)_0109_488.pdf` 의 정답도 직접 만들어 넣었다
(`result2.csv` 138줄 전부 `gt_found=1`). 아래는 문서를 새로 넣을 때 쓰는 절차다.
...
- **텍스트 레이어가 없는 파일** — 0자로 뽑히므로 정답을 직접 만들어야 한다.
- **대응 없는 글자(U+FFFD)** — PDF 의 글꼴이 유니코드 매핑을 안 들고 있어서 생긴다.
  그 자리는 손으로 채워야 하고, 안 채우면 모든 엔진이 똑같이 틀린 걸로 잡힌다.

지금 `data/` 에서는 텍스트 레이어가 없는 파일이 `ch/(2020)_0109_488.pdf` 하나,
U+FFFD 가 `ch/(2020)_0381_884.pdf` 4개 · `ko/251223_...` 5개 · `ko/260120_...` 6개다.
**PDF 원본에서 뽑을 때 그렇다는 뜻이고, `data/gt/` 의 정답은 이미 다 고쳐 놨다.**
`--report` 는 계속 이 목록을 짚어주므로 이미 처리한 것으로 알고 넘어가면 된다.
...
| 파일 | 내용 |
| --- | --- |
| `result/result2.csv` | 문서 × 엔진 한 줄. 지표, 언어, 소요 시간, 쪽 수, 에러 메시지 |
| `result/result2_summary.csv` | 엔진별 집계 (`group=ALL` 전체 + 언어 폴더별 한 줄씩) |
| `result/pred/<engine>/<문서>.txt` | 인식 원문. 눈으로 확인하거나 재채점할 때 쓴다 |

> **지금 올라간 CSV 에는 시간이 없다.** 마지막 집계를 `--metrics-only` 로 다시 냈기
> 때문에 `n_pages`, `seconds`, `sec_per_page` 가 138줄 전부 비어 있고 요약의
> `total_seconds` 도 0 이다. 저장된 예측을 그냥 읽는 경로라 잴 시간이 없다.
> 속도를 보려면 OCR 부터 다시 돌려야 한다. 대시보드는 시간 칸이 비면 관련 표시를
> 알아서 접는다 (`2-2_document_ocr.py` 의 `has_timing`).
